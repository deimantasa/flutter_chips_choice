...
class ChipsChoiceState<T> extends State<ChipsChoice<T>> {
  ...
  EdgeInsetsGeometry get defaultChipMargin =>
      widget.wrapped ? ChipsChoice.defaultWrappedChipMargin : ChipsChoice.defaultScrollableChipMargin;

  /// default style for unselected choice item
  C2ChoiceStyle get defaultChoiceStyle => C2ChoiceStyle(margin: defaultChipMargin, color: theme.unselectedWidgetColor);

  /// default style for selected choice item
  C2ChoiceStyle get defaultActiveChoiceStyle => C2ChoiceStyle(margin: defaultChipMargin, color: theme.primaryColor);
  ...
  List<Widget?> get choiceChips {
    return List<Widget?>.generate(choiceItems!.length, choiceChipsGenerator).where((e) => e != null).toList();
  }

  /// choice chips generator
  Widget? choiceChipsGenerator(int i) {
    final C2Choice<T> item = choiceItems![i].copyWith(
      selected: widget._isMultiChoice ? widget._values!.contains(choiceItems![i].value) : widget._value == choiceItems![i].value,
...
class C2Placeholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Text(message, textAlign: align ?? C2Placeholder.defaultAlign, style: C2Placeholder.defaultStyle.merge(style)),
    );
  }
}
